...
int main(int argc, char* argv[])
{
    ...
    float* audio_buffer = new float[44100 * 5]; // Fréquence d'échantillonnage * 5 (= 5 secondes de son) 
    const float freq = 440.0f; // 440 hz -> tonalité du téléphone 
    for (size_t i = 0; i < 44100 * 5; ++i)
    {
    ...
    std::optional<WAVParser::WAV_INFO> serialised = WAVParser::Serialise(header, reinterpret_cast<char*>(audio_buffer), 44100 * 5);
    WAVParser::Write("../res/tes.wav", serialised.value());
}
